...
{
    public class UInt32Converter : IDataConverter
    {
        public DataFormat Format => DataFormat.UInt32;
        ...
        public byte[] ConvertRequest(string data, bool isBigEndian, bool clrf)
        {
            ...
            {
                uint v = uint.Parse(parts[i], CultureInfo.InvariantCulture);
        ...
        public string ConvertResponse(byte[] data, bool isBigEndian)
        {
            ...
            for (int i = 0; i < data.Length / 4; i++)
            {
                ...
                uint v = isBigEndian ? BinaryPrimitives.ReadUInt32BigEndian(span) : BinaryPrimitives.ReadUInt32LittleEndian(span);
                if (i > 0) sb.Append(' ');
